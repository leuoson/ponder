...
export function getSystemPromptFilePath(cwd: string, mode: Mode): string {
	return path.join(cwd, ".ponder", `system-prompt-${mode}`)
}

/**
 * Loads custom system prompt from a file at .ponder/system-prompt-[mode slug]
...
export async function ensureRooDirectory(cwd: string): Promise<void> {
	const rooDir = path.join(cwd, ".ponder")
