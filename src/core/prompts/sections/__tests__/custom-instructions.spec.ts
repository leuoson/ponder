...
describe("loadRuleFiles", () => {
	...
	it("should read and trim file content", async () => {
		// Simulate no .ponder/rules directory
	...
	it("should use .ponder/rules/ directory when it exists and has files", async () => {
		// Simulate .ponder/rules directory exists
		...
		readdirMock.mockResolvedValueOnce([
			{
				...
				parentPath: "/fake/path/.ponder/rules",
			},
			{
				name: "file2.txt",
				isFile: () => true,
		...
		statMock.mockImplementation((path) => {
			...
			if (
				normalizedPath.includes("/fake/path/.ponder/rules/file1.txt") ||
				normalizedPath.includes("/fake/path/.ponder/rules/file2.txt")
			) {
		...
		readFileMock.mockImplementation((filePath: PathLike) => {
			...
			if (normalizedPath === "/fake/path/.ponder/rules/file1.txt") {
				return Promise.resolve("content of file1")
			}
			if (normalizedPath === "/fake/path/.ponder/rules/file2.txt") {
		...
		const result = await loadRuleFiles("/fake/path")
		const expectedPath1 =
			process.platform === "win32" ? "\\fake\\path\\.roo\\rules\\file1.txt" : "/fake/path/.ponder/rules/file1.txt"
		const expectedPath2 =
			process.platform === "win32" ? "\\fake\\path\\.roo\\rules\\file2.txt" : "/fake/path/.ponder/rules/file2.txt"
		...
		const expectedRulesDir = process.platform === "win32" ? "\\fake\\path\\.roo\\rules" : "/fake/path/.ponder/rules"
	...
	it("should filter out cache files from .ponder/rules/ directory", async () => {
		// Simulate .ponder/rules directory exists
		statMock.mockResolvedValueOnce({
			isDirectory: vi.fn().mockReturnValue(true),
		} as any)
		...
		readFileMock.mockImplementation((filePath: PathLike) => {
			...
			if (normalizedPath === "/fake/path/.ponder/rules/rule1.txt") {
				return Promise.resolve("rule 1 content")
			}
			if (normalizedPath === "/fake/path/.ponder/rules/rule2.md") {
			...
			if (normalizedPath === "/fake/path/.ponder/rules/.DS_Store") {
				return Promise.resolve("DS_STORE_BINARY_CONTENT")
			}
			if (normalizedPath === "/fake/path/.ponder/rules/Thumbs.db") {
				return Promise.resolve("THUMBS_DB_CONTENT")
			}
			if (normalizedPath === "/fake/path/.ponder/rules/backup.bak") {
				return Promise.resolve("BACKUP_CONTENT")
			}
			if (normalizedPath === "/fake/path/.ponder/rules/cache.log") {
				return Promise.resolve("LOG_CONTENT")
			}
			if (normalizedPath === "/fake/path/.ponder/rules/temp.tmp") {
				return Promise.resolve("TEMP_CONTENT")
			}
			if (normalizedPath === "/fake/path/.ponder/rules/script.pyc") {
		...
		const expectedCacheFiles = [
			"/fake/path/.ponder/rules/.DS_Store",
			"/fake/path/.ponder/rules/Thumbs.db",
			"/fake/path/.ponder/rules/backup.bak",
			"/fake/path/.ponder/rules/cache.log",
			"/fake/path/.ponder/rules/temp.tmp",
			"/fake/path/.ponder/rules/script.pyc",
		]
	...
	it("should fall back to .roorules when .ponder/rules/ is empty", async () => {
		// Simulate .ponder/rules directory exists
		statMock.mockResolvedValueOnce({
			isDirectory: vi.fn().mockReturnValue(true),
		} as any)
	...
	it("should read files from nested subdirectories in .ponder/rules/", async () => {
		// Simulate .ponder/rules directory exists
		statMock.mockResolvedValueOnce({
			isDirectory: vi.fn().mockReturnValue(true),
		} as any)
		...
		readdirMock.mockResolvedValueOnce([
			...
			},
			{
				...
				isSymbolicLink: () => false,
				isDirectory: () => false,
				parentPath: "/fake/path/.ponder/rules/subdir",
			},
			{
				name: "nested2.txt",
				isFile: () => true,
				isSymbolicLink: () => false,
				isDirectory: () => false,
				parentPath: "/fake/path/.ponder/rules/subdir/subdir2",
			},
		] as any)
		...
		readFileMock.mockImplementation((filePath: PathLike) => {
			...
			// Handle both Unix and Windows path separators
			const normalizedPath = pathStr.replace(/\\/g, "/")
			if (normalizedPath === "/fake/path/.ponder/rules/root.txt") {
				return Promise.resolve("root file content")
			}
			if (normalizedPath === "/fake/path/.ponder/rules/subdir/nested1.txt") {
				return Promise.resolve("nested file 1 content")
			}
			if (normalizedPath === "/fake/path/.ponder/rules/subdir/subdir2/nested2.txt") {
		...
		const expectedRootPath =
			process.platform === "win32" ? "\\fake\\path\\.roo\\rules\\root.txt" : "/fake/path/.ponder/rules/root.txt"
		const expectedNested1Path =
			process.platform === "win32"
				? "\\fake\\path\\.roo\\rules\\subdir\\nested1.txt"
				: "/fake/path/.ponder/rules/subdir/nested1.txt"
		const expectedNested2Path =
			process.platform === "win32"
				? "\\fake\\path\\.roo\\rules\\subdir\\subdir2\\nested2.txt"
				: "/fake/path/.ponder/rules/subdir/subdir2/nested2.txt"
...
describe("addCustomInstructions", () => {
	...
	it("should combine all instruction types when provided", async () => {
		// Simulate no .ponder/rules-test-mode directory
		statMock.mockRejectedValueOnce({ code: "ENOENT" })
	...
	it("should use .ponder/rules-test-mode/ directory when it exists and has files", async () => {
		// Simulate .ponder/rules-test-mode directory exists
		statMock.mockResolvedValueOnce({
			isDirectory: vi.fn().mockReturnValue(true),
		} as any)
		...
		readdirMock.mockResolvedValueOnce([
			{
				...
				isFile: () => true,
				isSymbolicLink: () => false,
				parentPath: "/fake/path/.ponder/rules-test-mode",
			},
			{
				name: "rule2.txt",
				isFile: () => true,
		...
		statMock.mockImplementation((path) => {
			...
			const normalizedPath = path.toString().replace(/\\/g, "/")
			if (
				normalizedPath.includes("/fake/path/.ponder/rules-test-mode/rule1.txt") ||
				normalizedPath.includes("/fake/path/.ponder/rules-test-mode/rule2.txt")
			) {
				return Promise.resolve({
					isFile: vi.fn().mockReturnValue(true),
				}) as any
		...
		readFileMock.mockImplementation((filePath: PathLike) => {
			...
			// Handle both Unix and Windows path separators
			const normalizedPath = pathStr.replace(/\\/g, "/")
			if (normalizedPath === "/fake/path/.ponder/rules-test-mode/rule1.txt") {
				return Promise.resolve("mode specific rule 1")
			}
			if (normalizedPath === "/fake/path/.ponder/rules-test-mode/rule2.txt") {
		...
		const expectedTestModeDir =
			process.platform === "win32" ? "\\fake\\path\\.roo\\rules-test-mode" : "/fake/path/.ponder/rules-test-mode"
		const expectedRule1Path =
			process.platform === "win32"
				? "\\fake\\path\\.roo\\rules-test-mode\\rule1.txt"
				: "/fake/path/.ponder/rules-test-mode/rule1.txt"
		const expectedRule2Path =
			process.platform === "win32"
				? "\\fake\\path\\.roo\\rules-test-mode\\rule2.txt"
				: "/fake/path/.ponder/rules-test-mode/rule2.txt"
	...
	it("should fall back to .roorules-test-mode when .ponder/rules-test-mode/ does not exist", async () => {
		// Simulate .ponder/rules-test-mode directory does not exist
		statMock.mockRejectedValueOnce({ code: "ENOENT" })
	...
	it("should fall back to .clinerules-test-mode when .ponder/rules-test-mode/ and .roorules-test-mode do not exist", async () => {
		// Simulate .ponder/rules-test-mode directory does not exist
		statMock.mockRejectedValueOnce({ code: "ENOENT" })
	...
	it("should correctly format content from directories when using .ponder/rules-test-mode/", async () => {
		...
		readdirMock.mockResolvedValueOnce([
			{ name: "rule1.txt", isFile: () => true, parentPath: "/fake/path/.ponder/rules-test-mode" },
		] as any)
		readFileMock.mockReset()
...
describe("Rules directory reading", () => {
	it.skipIf(process.platform === "win32")("should follow symbolic links in the rules directory", async () => {
			...
			.mockResolvedValueOnce([
				{
					...
					isFile: () => true,
					parentPath: "/fake/path/.ponder/rules/symlink-target-dir",
				},
			] as any)
		...
		statMock.mockImplementation((path: string) => {
			// For directory check
			if (path === "/fake/path/.ponder/rules" || path.endsWith("dir")) {
				return Promise.resolve({
					isDirectory: vi.fn().mockReturnValue(true),
					isFile: vi.fn().mockReturnValue(false),
				} as any)
		...
		readFileMock.mockImplementation((filePath: PathLike) => {
			...
			// Handle both Unix and Windows path separators
			const normalizedPath = pathStr.replace(/\\/g, "/")
			if (normalizedPath === "/fake/path/.ponder/rules/regular.txt") {
				return Promise.resolve("regular file content")
			}
			if (normalizedPath === "/fake/path/.ponder/symlink-target.txt") {
				return Promise.resolve("symlink target content")
			}
			if (normalizedPath === "/fake/path/.ponder/rules/symlink-target-dir/subdir_link.txt") {
				return Promise.resolve("regular file content under symlink target dir")
			}
			if (normalizedPath === "/fake/path/.ponder/nested-symlink-target.txt") {
		...
		expect(readlinkMock).toHaveBeenCalledWith("/fake/path/.ponder/rules/link.txt")
		expect(readlinkMock).toHaveBeenCalledWith("/fake/path/.ponder/rules/link_dir")

		// Verify both files were read
		expect(readFileMock).toHaveBeenCalledWith("/fake/path/.ponder/rules/regular.txt", "utf-8")
		expect(readFileMock).toHaveBeenCalledWith("/fake/path/.ponder/symlink-target.txt", "utf-8")
		expect(readFileMock).toHaveBeenCalledWith(
			"/fake/path/.ponder/rules/symlink-target-dir/subdir_link.txt",
			"utf-8",
		)
		expect(readFileMock).toHaveBeenCalledWith("/fake/path/.ponder/nested-symlink-target.txt", "utf-8")
	})
	beforeEach(() => {
		vi.clearAllMocks()
	})
	...
	it.skipIf(process.platform === "win32")("should correctly format multiple files from directory", async () => {
		...
		// Simulate listing files
		readdirMock.mockResolvedValueOnce([
			{ name: "file1.txt", isFile: () => true, parentPath: "/fake/path/.ponder/rules" },
			{ name: "file2.txt", isFile: () => true, parentPath: "/fake/path/.ponder/rules" },
			{ name: "file3.txt", isFile: () => true, parentPath: "/fake/path/.ponder/rules" },
		] as any)
		...
		statMock.mockImplementation((path) => {
			...
			const normalizedPath = path.toString().replace(/\\/g, "/")
			expect([
				"/fake/path/.ponder/rules/file1.txt",
				"/fake/path/.ponder/rules/file2.txt",
				"/fake/path/.ponder/rules/file3.txt",
		...
		readFileMock.mockImplementation((filePath: PathLike) => {
			...
			if (normalizedPath === "/fake/path/.ponder/rules/file3.txt") {
		...
		const expectedFile3Path =
			process.platform === "win32" ? "\\fake\\path\\.roo\\rules\\file3.txt" : "/fake/path/.ponder/rules/file3.txt"
	...
	it("should return files in alphabetical order by filename", async () => {
		...
		readdirMock.mockResolvedValueOnce([
			{ name: "zebra.txt", isFile: () => true, parentPath: "/fake/path/.ponder/rules" },
			{ name: "alpha.txt", isFile: () => true, parentPath: "/fake/path/.ponder/rules" },
			{ name: "Beta.txt", isFile: () => true, parentPath: "/fake/path/.ponder/rules" }, // Test case-insensitive sorting
		] as any)
		...
		readFileMock.mockImplementation((filePath: PathLike) => {
			const pathStr = filePath.toString()
			const normalizedPath = pathStr.replace(/\\/g, "/")
			if (normalizedPath === "/fake/path/.ponder/rules/zebra.txt") {
				return Promise.resolve("zebra content")
			}
			if (normalizedPath === "/fake/path/.ponder/rules/alpha.txt") {
				return Promise.resolve("alpha content")
			}
			if (normalizedPath === "/fake/path/.ponder/rules/Beta.txt") {
		...
		const expectedAlphaPath =
			process.platform === "win32" ? "\\fake\\path\\.roo\\rules\\alpha.txt" : "/fake/path/.ponder/rules/alpha.txt"
		const expectedBetaPath =
			process.platform === "win32" ? "\\fake\\path\\.roo\\rules\\Beta.txt" : "/fake/path/.ponder/rules/Beta.txt"
		const expectedZebraPath =
			process.platform === "win32" ? "\\fake\\path\\.roo\\rules\\zebra.txt" : "/fake/path/.ponder/rules/zebra.txt"
	...
	it("should sort symlinks by their symlink names, not target names", async () => {
		...
		// First call: check if .ponder/rules directory exists
		statMock.mockResolvedValueOnce({
			isDirectory: vi.fn().mockReturnValue(true),
		} as any)
