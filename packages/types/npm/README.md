...
const extension = vscode.extensions.getExtension<RooCodeAPI>("PonderDev.ponder")
...
**NOTE:** To ensure that the `PonderDev.ponder` extension is activated before your extension, add it to the `extensionDependencies` in your `package.json`:

```json
"extensionDependencies": ["PonderDev.ponder"]
```
